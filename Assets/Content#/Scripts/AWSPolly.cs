...
public class AWSPolly : MonoBehaviour {
    ...
    void Awake()
    {
        credentials = new CognitoAWSCredentials("us-east-1:a5a1ecb5-62f8-430f-aef7-7953d0a8a08a", RegionEndpoint.USEast1);  //CognitoCredential로 AWSCredential설정
    }

    public void ButtonEvent(TMP_InputField text)  //버튼 클릭 이벤트
    {
      PollyTTS(text);
    }

    public async void PollyTTS(TMP_InputField text)  //Polly 실행
    {
        if (text==null)
        {
            return;
       ...
       else if (!isProcess)
        {
            ...
            client = new AmazonPollyClient(credentials, RegionEndpoint.USEast1);  //아마존 Polly 클라이언트 설정
            ...
            audio_path = Application.persistentDataPath + "/" + (int)(System.DateTime.UtcNow.Subtract(new System.DateTime(1970, 1, 1))).TotalSeconds + "Ogg_vorbis";  //오디오 패스 설정
    ...
    IEnumerator PlayAudioClip(string audio_path)
    {
        using (UnityWebRequest www = UnityWebRequestMultimedia.GetAudioClip("file://" + audio_path, AudioType.OGGVORBIS))  //오디오 클립 가져오기
        {
            ...
            else
            {
                ...
                yield return new WaitForSeconds(40);  //실행중에 코루틴 일시정지
